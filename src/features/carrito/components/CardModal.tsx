...
export default function CardModal({ nombre, cantidad, precio, total }: Props) {
  ...
  return (
    <div className="flex items-center border border-white  justify-between rounded p-1">
      <article className="flex flex-col items-start justify-center">
        ...
        <p className="font-tajawal text-sm text-white/80">
          Cantidad: {cantidad}
        </p>
        <p className="font-tajawal text-sm text-white/80">
          Precio: {cantidad > 1 ? formatPrecio(total) : formatPrecio(precio)}
        </p>
      </article>
      <MdDeleteForever
        size={20}
        className="cursor-pointer text-white hover:text-red-300"
        onClick={() => deleteProduct(nombre)}
      />
    </div>
  );
}
